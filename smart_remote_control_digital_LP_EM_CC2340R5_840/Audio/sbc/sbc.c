...
#include <Audio/sbc/sbc_math.h>
#include <Audio/sbc/sbc_tables.h>

#include <Audio/sbc/sbc.h>
#include <Audio/sbc/sbc_private.h>
#include <Audio/sbc/sbc_primitives.h>
...
//	uint8_t joint;
	...
	int32_t SBC_ALIGNED sb_sample_f[16][8]; //[2][8];
#endif //MSBC_ENCODE

	/* modified subband samples */
	int32_t SBC_ALIGNED sb_sample[16][8]; //[2][8];
...
{
    #ifdef USE_BW_24KHz
    uint8_t sf = SBC_FREQ_48000;  //frame->frequency;
    #else
...
uint8_t sbc_mem[sizeof(struct sbc_priv) + SBC_ALIGN_MASK];
...
#ifndef SBC_CC26XX
    System_printf("Allocate %d bytes\n", sizeof(struct sbc_priv) + SBC_ALIGN_MASK);
    System_flush();
	sbc->priv_alloc_base = malloc(sizeof(struct sbc_priv) + SBC_ALIGN_MASK);
	if (!sbc->priv_alloc_base)
		return -SBC_ENOMEM;
#else
	sbc->priv_alloc_base = (void *)sbc_mem;
#endif //SBC_CC26XX
			...
			void *output, size_t output_len, size_t *written)
{
	...
	if (output_len < (size_t) (samples * 1/*priv->frame.channels*/ * 2))
		samples = output_len / (1/*priv->frame.channels*/ * 2);
...
//		}
	...
	if (written)
		*written = samples * 1/*priv->frame.channels*/ * 2;
			...
			void *output, size_t output_len, size_t *written)
{
...
#else
    sbc_enc_process_input = priv->enc_state.sbc_enc_process_input_8s_le;
#endif //ONLY_MSBC
	...
	return samples * 1/*priv->frame.channels*/ * 2;
}
#endif //MSBC_ENCODE
...
#endif //ONLY_MSBC
	channels = 1; //sbc->mode == SBC_MODE_MONO ? 1 : 2;
	joint = 0; //sbc->mode == SBC_MODE_JOINT_STEREO ? 1 : 0;
	bitpool = 26; //sbc->bitpool;
...
SBC_EXPORT unsigned sbc_get_frame_duration(sbc_t *sbc)
{
	...
	case SBC_FREQ_16000:
        ...
        #ifdef USE_BW_24KHz
	    frequency = 48000;  //frame->frequency;
        #else
        frequency = 16000; //frame->frequency;
        #endif
