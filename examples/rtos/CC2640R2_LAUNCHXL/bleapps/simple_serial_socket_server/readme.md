...
If you define `SHOW_BD_ADDR` in the `simple_serial_socket_server` project, the 
server will prints its own Bluetooth device address (BDA) when the program 
starts. This can be useful for debugging or if you are working in a busy RF 
environment. 
